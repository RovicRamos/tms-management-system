<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>TMS | Manage Sessions</title><link rel="icon" type="image/png" href="/images/logo.png"><script src="https://cdn.tailwindcss.com"></script>
<script>tailwind.config={theme:{extend:{colors:{darkSlate:'#082020',mutedTeal:'#5EAFBF',iceBlue:'#DDF2F7',softOrange:'#E57A44',terracotta:'#D1432A',deepOcean:'#0C4A60',crimson:'#D9534F',mustardGold:'#E6B800',slateGray:'#5B7582',softMutedTeal:'#4F9DA6'}}}}</script>
</head>
...
<section class="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-sm">
    <div class="flex items-center justify-between gap-6">
        <div class="space-y-2">
            <span class="inline-flex items-center text-xs font-bold tracking-wider text-deepOcean uppercase bg-iceBlue px-3 py-1 rounded-full">Sessions</span>
            <h1 class="text-3xl sm:text-4xl font-extrabold text-darkSlate tracking-tight">Manage Sessions</h1>
            <p class="text-slateGray text-sm sm:text-base">Create and maintain event sessions.</p>
        </div>
        <a href="{{ route('admin.sessions.create') }}" class="bg-softOrange text-white rounded-xl px-4 py-3 text-sm font-semibold hover:bg-orange-600 transition whitespace-nowrap">+ New Session</a>
    </div>
</section>

<section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="overflow-x-auto"><table class="min-w-full text-sm"><thead class="bg-gray-50 text-left text-xs uppercase tracking-wider text-slateGray"><tr><th class="px-4 py-3">Session</th><th class="px-4 py-3">Event</th><th class="px-4 py-3">Location</th><th class="px-4 py-3">Start</th><th class="px-4 py-3">Actions</th></tr></thead><tbody class="divide-y divide-gray-100">@forelse($sessions as $session)<tr><td class="px-4 py-4 font-semibold text-darkSlate">{{ $session->session_title ?? 'Untitled Session' }}</td><td class="px-4 py-4">{{ $session->event?->title ?? 'N/A' }}</td><td class="px-4 py-4">{{ $session->location }}</td><td class="px-4 py-4">{{ $session->start_date }}</td><td class="px-4 py-4 space-x-3"><a class="font-semibold text-softMutedTeal" href="{{ route('admin.sessions.edit', $session) }}">Edit</a><form action="{{ route('admin.sessions.destroy', $session) }}" method="POST" class="inline">@csrf @method('DELETE')<button class="font-semibold text-crimson" onclick="return confirm('Delete this session?')">Delete</button></form></td></tr>@empty<tr><td class="px-4 py-8 text-slateGray" colspan="5">No sessions found.</td></tr>@endforelse</tbody></table></div>
</section>
</main>
</body>
</html>