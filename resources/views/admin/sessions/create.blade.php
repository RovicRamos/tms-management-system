<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>TMS | New Session</title><link rel="icon" type="image/png" href="/images/logo.png"><script src="https://cdn.tailwindcss.com"></script>
...
<section class="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-sm">
    <div class="space-y-2">
        ...
        <h1 class="text-3xl sm:text-4xl font-extrabold text-darkSlate tracking-tight">Create Session</h1>
        <p class="text-slateGray text-sm sm:text-base">Add a session for an event.</p>
    </div>
</section>
<section class="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-sm">
<form action="{{ route('admin.sessions.store') }}" method="POST" class="space-y-5">@csrf @include('admin.sessions._form', ['buttonText' => 'Create Session'])</form>
</section>
</main>
</body>
</html>