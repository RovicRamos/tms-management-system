<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>TMS | New Event</title><link rel="icon" type="image/png" href="/images/logo.png"><script src="https://cdn.tailwindcss.com"></script>
...
<section class="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-sm">
    <div class="space-y-2">
        ...
        <h1 class="text-3xl sm:text-4xl font-extrabold text-darkSlate tracking-tight">Create Event</h1>
        <p class="text-slateGray text-sm sm:text-base">Add a new event to the admin schedule.</p>
    </div>
</section>
<section class="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-sm">
<form action="{{ route('admin.events.store') }}" method="POST" class="space-y-5">@csrf @include('admin.events._form', ['buttonText' => 'Create Event'])</form>
</section>
</main>
</body>
</html>