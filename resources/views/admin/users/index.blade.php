...
<body class="bg-gray-50 font-sans antialiased text-gray-900 min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        @if (session('success'))
            <div class="rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 px-4 py-3 text-sm">{{ session('success') }}</div>
        @endif
        @if ($errors->any())
        ...
        <section class="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-sm">
            <div class="space-y-2">
                ...
                <h1 class="text-3xl sm:text-4xl font-extrabold text-darkSlate tracking-tight">Manage Users</h1>
                <p class="text-slateGray text-sm sm:text-base">Edit roles and account status.</p>
            </div>
        </section>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 text-left text-xs uppercase tracking-wider text-slateGray">
                        <tr><th class="px-4 py-3">Name</th><th class="px-4 py-3">Email</th><th class="px-4 py-3">Role</th><th class="px-4 py-3">Active</th><th class="px-4 py-3">Actions</th></tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                            <tr><td class="px-4 py-8 text-slateGray" colspan="5">No users found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
